...
def get_earthdata_credentials(username: str = None, password: str = None) -> Tuple[str, str]:
    ...
    try:
        ...
        if netrc_path.exists():
            ...
            auth_info = netrc_auth.authenticators('urs.earthdata.nasa.gov')
            if auth_info:
                logging.debug("Successfully loaded credentials from .netrc")
                return auth_info[0], auth_info[2]  # username, password
            else:
                logging.debug("No entry for 'urs.earthdata.nasa.gov' found in .netrc")
    ...
    raise ValueError("No NASA Earthdata credentials available. Provide username/password directly "
                     "or set up .netrc file with entry for 'urs.earthdata.nasa.gov'.")
...
def download_url(
    ...
    for retry in range(1, max_retries + 1):
        try:
            with _request.urlopen(url) as response:
                if response.status == 200:
                    logging.debug(f"downloading from {url} to {destfile}")
...
def decompress_file(input_filepath: _Path, delete_after_decompression: bool = False) -> Optional[_Path]:
    ...
    if extension not in [".gz", ".tar", ".tar.gz", ".Z"]:
        logging.warning(f"Input file extension [{extension}] not supported - must be .gz, .tar.gz or .tar to decompress")
        return None
    if extension == ".gz":
        ...
        if delete_after_decompression:
            logging.debug(f"Deleting {input_file.name}")
            input_file.unlink()
        return output_file
        ...
        if input_file.stem[-1] not in ["d", "n"]:  # RINEX 2 files: "d" observation data, "n" broadcast ephemerides
            logging.warning(f"Only decompression of RINEX files currently supported for .Z decompression")
            return None
        output_file = _hatanaka.decompress_on_disk(path=input_file, delete=delete_after_decompression).resolve()
        logging.debug(f"Decompression of {input_file.name} to {output_file.name} in {output_file.parent} complete")
        return output_file
...
def download_multiple_files_from_cddis(
    ...
    with _concurrent.futures.ThreadPoolExecutor() as executor:
        list(executor.map(
            ...
            _repeat(earthdata_password),  # password
        ))
